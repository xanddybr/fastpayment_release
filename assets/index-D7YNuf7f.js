(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))t(n);new MutationObserver(n=>{for(const a of n)if(a.type==="childList")for(const l of a.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&t(l)}).observe(document,{childList:!0,subtree:!0});function s(n){const a={};return n.integrity&&(a.integrity=n.integrity),n.referrerPolicy&&(a.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?a.credentials="include":n.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function t(n){if(n.ep)return;n.ep=!0;const a=s(n);fetch(n.href,a)}})();const c=window.location.hostname==="localhost"?"http://localhost:8080":"https://misturadeluz.com/agenda/api/public";let d="events";const i={selection:document.querySelector("#step-selection"),auth:document.querySelector("#step-1"),otp:document.querySelector("#step-2"),login:document.querySelector("#login")};document.querySelector("#user-name");document.querySelector("#user-phone");document.querySelector("#email");document.querySelector("#otp-code");document.querySelector("#btn-send-otp");document.querySelector("#btn-verify-otp");const h=async()=>{const e=window.location.pathname,o=window.location.hash;u(),e.includes("login.html")||o==="#login"?i.login&&i.login.classList.remove("hidden"):e.includes("admin")||o==="#admin"?await b()?y():window.location.href="login.html":(i.selection&&i.selection.classList.remove("hidden"),v())},b=async()=>{try{return(await fetch(`${c}/api/auth/check`,{credentials:"include"})).ok}catch{return!1}},u=()=>{Object.values(i).forEach(e=>e?.classList.add("hidden"))},f=e=>new Date(e).toLocaleDateString("pt-BR",{weekday:"long"}),v=async(e="",o="")=>{const s=document.querySelector("#events-container");if(s){s.innerHTML='<p class="text-center col-span-full text-slate-400">Buscando horários...</p>';try{const t=`${c}/api/schedules?slug=${e}&type=${o}`,a=await(await fetch(t,{credentials:"include"})).json();if(!a||a.length===0){s.innerHTML='<p class="text-center col-span-full text-slate-500 py-10">Nenhum horário disponível.</p>';return}s.innerHTML=a.map(l=>`
            <div class="bg-slate-950 border border-slate-800 p-6 rounded-3xl shadow-2xl hover:border-fuchsia-600 transition-all duration-300 group">
                ...
                <div class="space-y-2 mb-6 border-l-2 border-violet-600/30 pl-4">
                    <p class="text-sm text-slate-300 flex items-center gap-2">
                        <span class="text-violet-500">📅</span> ${f(l.scheduled_at)}, ${new Date(l.scheduled_at).toLocaleDateString("pt-BR")}
                    </p>
        ...
        `),e.className="max-w-7xl mx-auto px-6 pt-24 pb-10",window.changeAdminTab("inicio")};window.closeCrudModal=()=>{const e=document.querySelector("#modal-crud");e&&e.classList.add("hidden")};window.changeAdminTab=e=>{const o=document.querySelector("#events-container");switch(document.querySelectorAll("header nav button").forEach(t=>{t.classList.remove("text-blue-600","border-b-2","border-blue-600"),t.classList.add("text-slate-500"),(t.textContent?.toLowerCase().trim()===e.toLowerCase().trim()||e==="inicio"&&t.textContent?.toLowerCase()==="início")&&(t.classList.remove("text-slate-500"),t.classList.add("text-blue-600","border-b-2","border-blue-600")),(t.textContent?.toLowerCase().trim()===e.toLowerCase().trim()||e==="agenda"&&t.textContent?.toLowerCase()==="agenda")&&(t.classList.remove("text-slate-500"),t.classList.add("text-blue-600","border-b-2","border-blue-600")),(t.textContent?.toLowerCase().trim()===e.toLowerCase().trim()||e==="inscricoes"&&t.textContent?.toLowerCase()==="inscrições")&&(t.classList.remove("text-slate-500"),t.classList.add("text-blue-600","border-b-2","border-blue-600")),(t.textContent?.toLowerCase().trim()===e.toLowerCase().trim()||e==="historico"&&t.textContent?.toLowerCase()==="histórico")&&(t.classList.remove("text-slate-500"),t.classList.add("text-blue-600","border-b-2","border-blue-600"))}),e){case"inicio":const t=localStorage.getItem("admin_full_name")||"Administrador";o.innerHTML=`
            ...
            `,p(),m(),g();break;case"inscricoes":o.innerHTML=`
            ...
            `;break}};const p=async()=>{const e=document.querySelector("#adminTableBody");if(e)try{const s=await(await fetch(`${c}/schedules`,{credentials:"include"})).json(),t=new Date;e.innerHTML=s.map(n=>{const a=new Date(n.scheduled_at),l=a<t,r=l?"text-red-600 font-bold":"text-slate-500";return`
                <tr class="hover:bg-slate-50 transition-colors border-b border-slate-50">
                    <td class="p-4 font-medium ${r}">${f(n.scheduled_at)}</td>
                    <td class="p-4">
