(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))t(a);new MutationObserver(a=>{for(const n of a)if(n.type==="childList")for(const l of n.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&t(l)}).observe(document,{childList:!0,subtree:!0});function s(a){const n={};return a.integrity&&(n.integrity=a.integrity),a.referrerPolicy&&(n.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?n.credentials="include":a.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function t(a){if(a.ep)return;a.ep=!0;const n=s(a);fetch(a.href,n)}})();let c="events";const i={selection:document.querySelector("#step-selection"),auth:document.querySelector("#step-1"),otp:document.querySelector("#step-2"),login:document.querySelector("#login")};document.querySelector("#user-name");document.querySelector("#user-phone");document.querySelector("#email");document.querySelector("#otp-code");document.querySelector("#btn-send-otp");document.querySelector("#btn-verify-otp");const h=async()=>{new URLSearchParams(window.location.search);const o=window.location.pathname.replace("/agenda","")||"/";u(),o.includes("/admin")?await f()?v():window.location.href="/agenda/login":o.includes("/login")?i.login.classList.remove("hidden"):(i.selection.classList.remove("hidden"),b())},f=async()=>{try{return(await fetch("http://localhost:8080/api/auth/check",{credentials:"include"})).ok}catch{return!1}},u=()=>{Object.values(i).forEach(e=>e?.classList.add("hidden"))},x=e=>new Date(e).toLocaleDateString("pt-BR",{weekday:"long"}),b=async(e="",o="")=>{const s=document.querySelector("#events-container");if(s){s.innerHTML='<p class="text-center col-span-full text-slate-400">Buscando horários...</p>';try{const t=`http://localhost:8080/api/schedules?slug=${e}&type=${o}`,n=await(await fetch(t)).json();if(!n||n.length===0){s.innerHTML='<p class="text-center col-span-full text-slate-500 py-10">Nenhum horário disponível.</p>';return}s.innerHTML=n.map(l=>`
        ...
        `),e.className="max-w-7xl mx-auto px-6 pt-24 pb-10",window.changeAdminTab("inicio")};window.closeCrudModal=()=>{const e=document.querySelector("#modal-crud");e&&e.classList.add("hidden")};window.changeAdminTab=e=>{const o=document.querySelector("#events-container");switch(document.querySelectorAll("header nav button").forEach(t=>{t.classList.remove("text-blue-600","border-b-2","border-blue-600"),t.classList.add("text-slate-500"),(t.textContent?.toLowerCase().trim()===e.toLowerCase().trim()||e==="inicio"&&t.textContent?.toLowerCase()==="início")&&(t.classList.remove("text-slate-500"),t.classList.add("text-blue-600","border-b-2","border-blue-600")),(t.textContent?.toLowerCase().trim()===e.toLowerCase().trim()||e==="agenda"&&t.textContent?.toLowerCase()==="agenda")&&(t.classList.remove("text-slate-500"),t.classList.add("text-blue-600","border-b-2","border-blue-600")),(t.textContent?.toLowerCase().trim()===e.toLowerCase().trim()||e==="inscricoes"&&t.textContent?.toLowerCase()==="inscrições")&&(t.classList.remove("text-slate-500"),t.classList.add("text-blue-600","border-b-2","border-blue-600")),(t.textContent?.toLowerCase().trim()===e.toLowerCase().trim()||e==="historico"&&t.textContent?.toLowerCase()==="histórico")&&(t.classList.remove("text-slate-500"),t.classList.add("text-blue-600","border-b-2","border-blue-600"))}),e){case"inicio":const t=localStorage.getItem("admin_full_name")||"Seu nome de usuário não foi carregado!";o.innerHTML=`
            ...
            `;break}};const p=async()=>{const e=document.querySelector("#adminTableBody");if(e)try{const s=await(await fetch("http://localhost:8080/schedules",{credentials:"include"})).json(),t=new Date;e.innerHTML=s.map(a=>{const n=new Date(a.scheduled_at),l=n<t,r=l?"text-red-600 font-bold":"text-slate-500";return`
                <tr class="hover:bg-slate-50 transition-colors border-b border-slate-50">
                    <td class="p-4 font-medium ${r}">${x(a.scheduled_at)}</td>
                    
                    <td class="p-4">
                        <div class="${r}">${n.toLocaleDateString("pt-BR")} - ${n.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}</div>
                        <div class="text-[10px] text-slate-400"></div>
                    </td>
                    
                    <td class="p-4 font-bold ${l?"text-red-600":"text-slate-900"}">${a.event_name}</td>
